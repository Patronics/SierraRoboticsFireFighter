...
#ifdef motor_test
if w5<10 and timer>2 then             'backup if too close to wall
	gosub gobackward
'hpwmDuty 1023
elseif Rusrfval > 165 then
...
elseif Fusrfval<85 and timer>2 then      'if farther away but approaching, start gradual turn
...
if ldr1on=1 and timer > 2 then     ''''CHANGE BACK TO 4, 200 ONLY FOR TESTING!!
...
gosub getldr
ldr1val=returnb1
if ldr1val<ldrthresh and checkgreater=0 or ldr1val>ldrthresh and checkgreater=1 then
	ldr1on=1
else
	ldr1on=0
endif
...
gosub getldr
ldr2val=returnb1
if ldr2val<ldrthresh and checkgreater=0 or ldr2val>ldrthresh and checkgreater=1 then
	ldr2on=1
else
	ldr2on=0
endif
...
gosub getldr
ldr3val=returnb1
if ldr3val<ldrthresh and checkgreater=0 or ldr3val>ldrthresh and checkgreater=1 then
	ldr3on=1
else
	ldr3on=0
endif
