...
''''STARTING FROM FRESH, FOR A NEW PERSPECTIVE!
...
if Rusrfval > 165 and Lusrfval > 155 and state=0 then     '''both were 165     
	sertxd ("Turning right, 'infinite' distance seen")
	gosub steerright
	pause 1000
	state=1
elseif Lusrfval < 200 and state=1 then
	gosub goforward
	state=2
elseif Fusrfval < 80 and state=2 then
	gosub turnright
	
	state=3
	
elseif Fusrfval > 150 and state=3 then
	gosub goforward
	
	state=4
elseif ldr1on=1 and state=4 then
	state=5
	goto firescan
endif
